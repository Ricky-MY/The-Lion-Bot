...
class Authenticator(Cog):
    ...
    @commands.guild_only()
    async def authenticator(self, ctx, *, key: str):
        """
        ...
        if key in keys:
            ...
            embed = discord.Embed(
                title=f'ERROR...', description=f'Invalid Code -- please try again! \n`NOTE: This operation is Case Sensitve`', color=self.error_color)
            embed.set_footer(
                text=f'Usage: {ctx.prefix}{ctx.command.qualified_name} {ctx.command.signature}', icon_url=self.bot.user.avatar_url)
            await ctx.reply(embed=embed)
    ...
    async def create(self, ctx, limit: int = 1):
        """
        ...
        if limit <= 0 or limit > 100:
            embed = discord.Embed(
                title=f'ERROR...', description=f'You have entered an invalid limit!\nMake sure to set a limit between 1 to 100.', color=self.error_color)
            embed.set_footer(
                text=f'Usage: {ctx.prefix}{ctx.command.qualified_name} {ctx.command.signature}', icon_url=self.bot.user.avatar_url)
            await ctx.send(embed=embed)
        else:
            embed = discord.Embed(title=f'Keys successfully created!',
                                  description='```DISCLOSING ANY KEY(S) CAN RESULT IN SECURITY BREACHES```', color=self.color)
            all_keys = []
            count = 0
            while count != limit:
                key = self.get_otp(10)
    ...
    async def dump(self, ctx):
        ...
        channel = ctx.channel
        embed = discord.Embed(
            title=f'HOLD UPPP...', description=f'Type in `confirm` to confirm the deletion of all keys.', color=self.error_color)
    ...
    async def statistics(self, ctx):
        ...
        embed = discord.Embed(title=f'Authenticator statistics',
                              description=f'Total valid keys left : {len(keys)}', color=self.color)
        await ctx.send(embed=embed)
