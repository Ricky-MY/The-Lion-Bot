...
class Moderation(commands.Cog):
    ...
    async def kick(self, ctx, member: discord.Member, *, reason):
        """Removes a user from the server. Beware that the user can still re-join, consider using ban to restrict accessibility of this server to the user."""
    ...
    async def clear(self, ctx, amount=5, user: discord.Member = None):
        """Removes a certain amount of messages from the channel of which the command is used in. If the amount isn't specified, it will default to 5."""
        channel = ctx.channel
